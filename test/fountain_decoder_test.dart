...
void main() {
  group('Fountain Decoder', () {
    test('decoder', () {
      ...
      final encoder = FountainEncoder(message: message, maxFragmentLength: maxFragmentLength, firstSeqNum: 100);
    ...
    test('partition and join', () {
      ...
      final fragmentsHex = fragments.map((fragment) => HEX.encode(fragment)).toList();

      final expectedFragmentHexes = [
      ...
      expect(fragmentsHex, equals(expectedFragmentHexes));

      final rejoinedMessage = FountainDecoder.joinFragments(fragments, message.length);
      expect(rejoinedMessage, equals(message));
    });
  });
}
